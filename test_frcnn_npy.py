...
npy_file = '{}/img_patches.npy'.format(img_path)
all_data = np.load(npy_file)
data_shape = all_data.shape
num_images = data_shape[0]
img_names = []
img_file = '{}/img_names.txt'.format(img_path)
...
for idx, img_name in enumerate(img_names):
    # FIXME
    # if idx > 5:    # do a couple for testing purposes
    #     break
    ...
    for jdx in range(data_shape[1]):
        for kdx in range(data_shape[2]):
            img = all_data[idx, jdx, kdx, ]
            img = np.stack([img]*3, axis=2)     # pretend there are 3 channels

            # double check img
            img2 = cv2.imread('{}/{}'.format('../pepple/accell/segmentations', img_name), cv2.IMREAD_GRAYSCALE)
            ...
            for key in bboxes:
                ...
                if 'pepple' in img_path:
                    ...
                    real_img_name = '{}_h{}_w{}.png'.format(img_name.replace('.png', ''), int(jdx*32), int(kdx*32))
                    with open(coords_file, 'a') as fout:
                        fout.write('{}\t{}\n'.format(real_img_name, supp_coords))
                    with open(all_coords_file, 'a') as fout:
                        fout.write('{}\t{}\n'.format(real_img_name, all_coords))
...
# "-p" "../pepple/accell/seg_accell_img_patches" "--network" "vgg" "--config_filename" "../pepple/accell/blank_32_32_realistic/config.pickle"