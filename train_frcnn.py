...
epoch_log = './epoch_log.txt'
iter_log = './iter_log.txt'
...
if options.network == 'vgg':
    ...
    raise ValueError
print('network=%s; C.network=%s' % (options.network, C.network))
...
for epoch_num in range(num_epochs):
    ...
    while True:
        try:
            ...
            with open(iter_log, 'a') as fout:
                fout.write('%d\t%s\n' % (epoch_num*epoch_length+iter_num, '\t'.join([str(loss) for loss in losses[iter_num,:]])))

            iter_num += 1

            progbar.update(epoch_num*epoch_length+iter_num,
            ...
            if iter_num == epoch_length:
                ...
                with open(epoch_log, 'a') as fout:
                    fout.write('%d\t%f\t%f\t%f\t%f\t%f\t%f\n' % (epoch_num, loss_rpn_cls, loss_rpn_regr, loss_class_cls,
                                                           loss_class_regr, class_acc, curr_loss))
